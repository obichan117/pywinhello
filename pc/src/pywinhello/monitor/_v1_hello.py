"""HelloMonitor — WinEvent hook daemon for automatic PIN entry (v1 API).

Preserved for backward compatibility. The v2 replacement is
...
class HelloMonitor:
    ...
    def _handle_dialog(self) -> AuthEvent:
        ...
        1. Per-app config (AppConfig.pin) if the owner exe is whitelisted
        2. PYWINHELLO_PIN env var (global fallback via enter_pin)
        """
        owner_exe = dialog.get_owner_exe()
        app_cfg = self._get_app_config(owner_exe)

        pin = app_cfg.pin if app_cfg else None
        ...
        try:
            ...
            logger.warning("Skipping dialog for %s: %s", owner_exe, e)
            return AuthEvent(owner_exe=owner_exe, error=str(e))
