...
class AppConfig:
    ...
    pin_select_keys: list[str] = field(default_factory=lambda: ["ESCAPE"])
    """Key sequence to navigate from fingerprint to PIN mode (fallback).
    Only used when the dialog opens in fingerprint mode.
    Default ['ESCAPE'] dismisses fingerprint, falling back to PIN.
    """
